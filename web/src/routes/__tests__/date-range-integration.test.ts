...
describe('Date Range URL Parameter Syncing (Integration)', () => {
	describe('URL parameter persistence across pages', () => {
		it('should preserve start and end date parameters when navigating between tabs', () => {
			...
			const overviewURL = new URL(
				`http://localhost/?tab=overview&start=${startDate}&end=${endDate}`
			);

			// Simulate navigating to different tab while preserving parameters
			const economicsURL = new URL(
				`http://localhost/?tab=platform-economics&start=${startDate}&end=${endDate}`
			);
		...
		it('should allow clearing date parameters', () => {
			const urlWithParams = new URL(
				'http://localhost/?tab=overview&start=2026-01-01&end=2026-05-31'
			);
	...
	describe('Multi-tab date range awareness', () => {
		it('should support different date ranges for different queries', () => {
			...
			const economicsUrl = new URL(
				'http://localhost/?tab=platform-economics&start=2026-01-01&end=2026-03-31'
			);
	...
	describe('Date range and other filters together', () => {
		it('should preserve date range with channel filter', () => {
			const url = new URL(
				'http://localhost/?tab=sales&channel=swiggy&start=2026-01-01&end=2026-05-31'
			);
		...
		it('should preserve date range with status filter', () => {
			const url = new URL(
				'http://localhost/?tab=order-journal&status=delivered&start=2026-01-01&end=2026-05-31'
			);
		...
		it('should preserve date range with pagination', () => {
			const url = new URL(
				'http://localhost/?tab=orders&page=2&limit=25&start=2026-01-01&end=2026-05-31'
			);
	...
	describe('URL encoding and special characters', () => {
		it('should handle URL with special characters in other parameters', () => {
			const url = new URL(
				'http://localhost/?tab=orders&search=customer%20name&start=2026-01-01&end=2026-05-31'
			);
