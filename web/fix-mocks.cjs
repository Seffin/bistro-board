...
testFiles.forEach((file) => {
	...
	content = content.replace(
		/from: vi\.fn\(\)\.mockImplementation\(\(table: any\) => \{\s+if \(table && table\[Symbol\.for\('drizzle:Name'\)\] === 'expenses'\) \{\s+return Promise\.resolve\(\[\]\);\s+\}\s+return Promise\.resolve\(mockOrders\);\s+\}\)/g,
	...
	content = content.replace(
		/from: vi\.fn\(\)\.mockReturnValue\(Promise\.resolve\(\[\]\)\)/g,
		`from: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue([]), then: (res) => res([]) })`
	);
	content = content.replace(
		/from: vi\.fn\(\)\.mockReturnValue\(Promise\.resolve\(mockOrders\)\)/g,
		`from: vi.fn().mockReturnValue({ where: vi.fn().mockResolvedValue(mockOrders), then: (res) => res(mockOrders) })`
	);
